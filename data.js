...
const BUDGET_TEXT_RAW = [
  { t: '{{student_price}}.' },
  { t: 'Заявленный потолок — ${{budget}} в месяц. Всё, что дороже, помечай как «позже», а не как «нельзя».', when_: 'budget' }
];
const BUDGET_TEXT = [];
...
const MARKET_RAW = [
  ...
  { dir: 'SOC в {{hub}} (гибрид или переезд)', real: 'high', text: 'Самое реалистичное направление: {{employers}}. Требования таких вакансий 1:1 совпадают с этим планом — читай их сейчас, а не в W48, и правь по ним приоритеты.' },
  ...
  { dir: 'Стажировка через {{edu}}', real: 'high', text: 'Недооценённый канал. Учебное заведение обязано организовывать практику — направь этот механизм сам, с конкретным списком компаний, а не жди распределения по остаточному принципу.' }
];
const MARKET = [];
...
{{city}} (готов к переезду в {{hub}} / удалённой работе / сменному графику)
