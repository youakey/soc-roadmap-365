...
const DAILY = [
  { id: 'polish',  name: 'Polish',            min: 10,  desc: 'Duolingo (курс с английского!) + Anki PL. Мозг ещё холодный — идеально для механической памяти.' },
...
const RULES = [
  ...
  { name: '5 дней, не 7', text: 'Два дня в неделю полностью без кибербезопасности — это не лень, это условие работоспособности на дистанции 52 недель.' },
  ...
  { name: 'Физика', text: 'Правило 20-20-20 для глаз, 5 минут разминки между блоками, 2 тренировки в неделю вне 15 часов. Без этого план не доживёт до Q4.' }
];

const RED_FLAGS = [
  'Пропущено 3+ дня подряд без внешней причины',
  'Открываешь материал и не можешь читать дольше 10 минут',
  'Раздражение при мысли о ноутбуке',
  'Падение оценок в универе',
  'Нарушение сна',
  '«Я ничего не успеваю и всё равно ничего не знаю» как фоновая мысль'
];
...
const BUDGET = {
  ...
  recommended: [
    { item: 'LetsDefend / HTB (проверь Student Pricing)', when: 'W43–W44', cost: '$20–30/мес × 2', prio: 'Высокий' },
...
const MARKET = [
  { dir: 'SOC в Бресте', real: 'low', text: 'В городе нет операционных SOC-центров. Промежуточный шаг — helpdesk / сисадмин с уклоном в ИБ. Это нормально и работает.' },
  { dir: 'SOC в Минске (гибрид/переезд)', real: 'high', text: 'Самое реалистичное. Банки (Беларусбанк, БНБ, Приорбанк, Альфа), телеком (А1, МТС, Белтелеком), интеграторы. Требования вакансий 1:1 совпадают с этим планом.' },
  { dir: 'Удалённая L1-смена (СНГ)', real: 'mid', text: 'MSSP и аутсорс-SOC часто нанимают удалённо на ночные смены. Ночные смены совместимы с универом — это твой козырь как студента.' },
  { dir: 'Польша / ЕС SSC', real: 'low', text: 'Требуют English B2 и чаще всего право на работу в ЕС. Реальная цель на Year 2, а не на W52.' },
  { dir: 'Стажировка от университета', real: 'high', text: 'Недооценённый канал. Университет обязан организовывать практику — направь этот механизм, а не жди распределения по остаточному принципу.' }
];

const OUTCOMES = [
  { s: 'Offer Junior SOC Analyst / L1', p: '25–35%', text: 'Отличный, но не гарантированный исход за 1 год с уровня 2/5' },
  { s: 'Стажировка / практика в ИБ', p: '~45%', text: 'Полностью достаточный результат. Через 6 мес конвертируется в позицию' },
  { s: 'Смежная позиция (helpdesk, NOC)', p: '~60%', text: 'Нормальный и очень частый путь входа в SOC' },
  { s: 'Портфолио и B1 English без оффера', p: '~15%', text: 'Не провал. Year 2 с этой базой почти всегда закрывается оффером' }
];

const CV_TEXT = `[Имя Фамилия] | Junior SOC Analyst
Брест, Беларусь (готов к переезду в Минск / удалённой работе / сменному графику)
...
EDUCATION
[Университет], Information Security, 3rd year

LANGUAGES
Russian - native | Belarusian - native | English - B1 (technical reading B2)
Polish - A2`;
...
const COLD_EMAIL = `Тема: Junior SOC Analyst — студент ИБ с собственной лабораторией мониторинга

Здравствуйте, [Имя]!

Меня зовут [Имя], я студент 3 курса по специальности «Информационная
безопасность» в [ВУЗ], Брест.
...
const APP_CATEGORIES = ['Минск-офис', 'Удалёнка СНГ', 'Польша / ЕС', 'Брест / локально', 'Стажировка / практика', 'Холодное письмо'];
