...
const Person = {
  ...
  /** Ключи, которых нет в анкете. Диагностика, а не защита: защита
   *  уже стоит и работает — `parse()` перекладывает поля поимённо,
   *  и чужой ключ до состояния не доходит вовсе. Но доходит он
   *  МОЛЧА, и вот это и есть дефект: сервер имён параметров
   *  намеренно не знает (§11.5 про дублированный список аватаров),
   *  значит ловить опечатку больше некому.
   *
   *  Зовётся ровно на одном пути — при чтении из базы. Ни `set()`,
   *  ни местный кеш сюда не приходят: `set()` собирает объект сам
   *  и чужого ключа туда не кладёт, а кеш — это наша же вчерашняя
   *  запись. Проверка, которая шумит, не работает (§13.2-bis),
   *  поэтому она стоит там, где чужой ключ действительно может
   *  появиться, и больше нигде.
   *
   *  Ничего не выбрасывается и не падает: параметр, которого клиент
   *  не понимает, мог прийти от БОЛЕЕ НОВОГО клиента на другом
   *  устройстве. Стереть его здесь значило бы, что старый браузер
   *  тихо обрезает анкету, заполненную в новом. */
  strays(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return [];
    const known = {};
    PERSON_FIELDS.forEach(f => { known[f.id] = 1; });
    const out = [];
    Object.keys(raw).forEach(k => {
      if (!Object.prototype.hasOwnProperty.call(known, k)) out.push(k);
    });
    return out;
  },
  ...
  async load(sb, user) {
    ...
    try {
      ...
      /* Чужие ключи — только сказать, и сказать в двух местах:
         в консоль (её читает тот, кто чинит) и в `Person.note`
         (его видно из кода и стенда). Имена печатаются целиком:
         «есть лишний ключ» без имени не чинится. */
      const stray = this.strays(cloud);
      if (stray.length) {
        this.note = `в параметрах ${stray.length} незнакомых ключей: ${stray.join(', ')}`;
        console.warn('person: ' + this.note +
          ' — они не применяются. Либо опечатка в имени, либо параметр от более нового клиента.');
      }
