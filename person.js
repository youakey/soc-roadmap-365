...
/* ── Анкета ───────────────────────────────────────────────────
   Двадцать два поля. Больше §6 не спрашивает и меньше нельзя:
...
const PERSON_FIELDS = [
  /* Точка отсчёта плана. Стоит первой, потому что спрашивается
     первой: пока её нет, человек смотрит на чужие даты.

     Это ПАРАМЕТР ЧЕЛОВЕКА, а не трека, и граница здесь та же, что
     у §13.2-bis: трек задаёт форму (52 недели, темы, часы), человек
     задаёт, когда он по этой форме идёт. Держать дату в треке
     значило бы, что второй человек не может начать в свой день,
     не сдвинув план первому.

     Пусто — трек остаётся на собственной дате. Это не «поле
     не заполнено», а осмысленное значение: у трека своя дата есть
     всегда, и она годится (§12.7). */
  { id: 'start',      label: 'START',     type: 'date', max: 10,  def: '',  hint: 'начало курса — лучше понедельник' },
  /* Два поля вместо одного, и это не педантизм. В шапку CV просится
     ФИО целиком — так принято, и «Рыжов Иосиф Александрович | Junior
     SOC Analyst» читается правильно. Тем же значением холодное письмо
     говорит «Меня зовут Рыжов Иосиф Александрович», а это звучит как
     заявление в деканат: там нужно одно имя. Одно поле не может быть
     обоими.

     Выводить в письме первое слово из ФИО — это гадание о порядке
     слов: «Иосиф Рыжов» и «Рыжов Иосиф» встречаются оба, и ошибётся
     оно ровно на половине людей, молча. Поэтому спрашиваем, а не
     угадываем. Пустое `name_short` откатывается на `name` — то есть
     у того, кто заполнил только одно поле, ничего не меняется. */
  { id: 'name',       label: 'NAME',      type: 'text', max: 60,  def: '',  hint: 'ФИО целиком — шапка CV' },
  { id: 'name_short', label: 'CALL ME',   type: 'text', max: 40,  def: '',  hint: 'как представляться в письме' },
  ...
  /* Тот же класс, что `name`: одно поле, два языка вывода. В блоке
     EDUCATION английского CV стояло «БрГТУ, Information Security» —
     русская аббревиатура в английском абзаце. Читателю этого CV она
     не говорит ничего, а рекрутеру за границей — тем более.
     Пустое поле откатывается на `edu`, то есть у того, кто его
     не заполнил, всё как было. */
  { id: 'edu_en',    label: 'EDU / EN',   type: 'text', max: 80,  def: '',  hint: 'то же по-английски, для CV' },
...
const Person = {
  ...
  parse(raw) {
    ...
    PERSON_FIELDS.forEach(f => {
      ...
      /* Форма И существование, одним `isDay` на весь проект
         (security.js). Пустая строка — законное значение и означает
         «оставить дату трека», а не «поле не заполнено». */
      if (f.type === 'date') {
        if (v === '') { out[f.id] = ''; return; }   // законное «дата трека»
        if (!isDay(v)) return;
        out[f.id] = v;
        return;
      }
  ...
  /** Взять из `cloud` значения только для тех полей, которые
   *  в `local` ПУСТЫ. Пустое поле человек не редактировал, значит
   *  здесь нечего терять. Непустое не трогается никогда — иначе
   *  это было бы затирание неотправленной правки.
   *
   *  Пустым считается `''`, `null` и `undefined`. Ноль и `false`
   *  пустыми НЕ считаются: это введённые значения, и подменять их
   *  облаком нельзя. Поля перебираются по анкете, а не по ключам
   *  объекта, — чужой ключ так не проберётся (та же дисциплина,
   *  что у `parse`). */
  fillEmpty(local, cloud) {
    const out = Object.assign({}, local);
    if (!cloud || typeof cloud !== 'object' || Array.isArray(cloud)) return out;
    const empty = v => v === '' || v === null || v === undefined;
    PERSON_FIELDS.forEach(f => {
      if (!empty(own(out, f.id, ''))) return;
      const c = own(cloud, f.id, undefined);
      if (!empty(c)) out[f.id] = c;
    });
    return out;
  },
  ...
  vars() {
    ...
    /* Откат «короткое → полное → скобка» пишется здесь, а не в шаблоне:
       шаблон обязан оставаться текстом, а это правило. Скобка вместо
       пустоты — конвенция §13.2-bis: незаполненное поле показывается
       как `[название]` и стоит после разделителя. */
    const nameFull = s('name', '');
    const nameShort = s('name_short', '') || nameFull;
    const eduRu = s('edu', '');

    return {
      name:   nameFull || '[Имя]',
      /* Как представляться. Первое слово из ФИО НЕ берётся: порядок
         слов у людей разный, и угадывание ошибётся молча. */
      name_short: nameShort || '[Имя]',
      city:   s('city', '[город]'),
      ...
      edu:    eduRu || '[учебное заведение]',
      /* Английский блок CV. Откат на русское значение честнее пустоты:
         «БрГТУ» в английском абзаце плохо, но пустая строка в графе
         EDUCATION хуже — её читают как «образования нет». */
      edu_en: s('edu_en', '') || eduRu || '[university]',
  ...
  async load(sb, user) {
    ...
    try {
      ...
      if (takeCloud) {
        ...
        /* МЕСТНОЕ ПОБЕДИЛО — но не целиком, и это починка тупика,
           найденного 02.09.2026 по скриншоту владельца.

           Что было. `loadLocal()` для кеша СТАРОГО образца ставит
           `dirty = true` (строка `own(box, 'dirty', true)` при
           `boxed === false` даёт `!!raw`, то есть true). Правило 2
           выше отдаёт победу местному при `dirty` ВСЕГДА — а снять
           этот флаг может только удачный `push()`, которого никто
           не звал: `push` идёт лишь из `schedule()`, то есть после
           правки поля в SETTINGS. Устройство, ни разу не менявшее
           анкету после обновления, оставалось с `dirty = true`
           навсегда, и облачные параметры не приезжали НИКОГДА.
           Снаружи это выглядело так: дата старта стоит в базе,
           а недели на экране показывают даты трека.

           Почему нельзя просто отдать победу облаку: правило 2
           защищает неотправленные правки, и ломать его значит
           терять напечатанное без сети.

           Почему нельзя просто отправить местное: у застрявшего
           устройства местное БЕДНЕЕ облачного, и такая отправка
           затёрла бы на сервере то, чего на устройстве нет.

           Поэтому третий путь: местное побеждает, а ПУСТЫЕ поля
           добираются из облака. Пустое поле никто не редактировал,
           значит забрать туда облачное значение нельзя ничего
           потерять — а полное слияние по полям невозможно, меток
           на поле сервер не хранит (§13.2-bis).

           И отправка следом, чтобы `dirty` перестал быть вечным:
           без неё тупик снимался бы только вручную. Не `await` —
           вход не должен ждать необязательного круга. */
        const merged = this.parse(this.fillEmpty(this.p, cloud));
        const grew = JSON.stringify(merged) !== JSON.stringify(this.p);
        this.p = merged;
        this.source = 'local';
        if (grew) this.saveLocal();
        if (this.dirty) this.push(sb, user);
      }
    } catch (e) {
...
function applyPerson(reseat) {
  /* Даты пересаживаются ЗДЕСЬ, а не отдельным вызовом рядом, и это
     прямое исполнение мины §12.1-ter: правило, зашитое в двух местах,
     однажды правят в одном. Вызовов `applyPerson()` три — загрузка
     файла, `openApp()` и правка параметра в SETTINGS, — и все три
     обязаны получить одинаковые даты. Один вызов внутри вместо трёх
     снаружи означает, что забыть негде.

     Дёшево это потому, что `rebase()` идемпотентна: она считает
     даты от НОМЕРА недели, а не сдвигает существующие (§12.7). */
  Content.rebase(Person.val('start'));
