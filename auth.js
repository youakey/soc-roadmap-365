...
/* Ключ первого экрана. Своя запись, а не поле в `Store.d`: тот
   целиком уезжает в `progress.payload` и приезжает обратно слиянием
   (§12.5-bis), а «видел ли ЭТОТ браузер первый экран» — свойство
   устройства, и в облаке ему делать нечего. Соседи по этому правилу:
   `soc365.track.v1` и `soc365.content.v1`. */
const GATE_SEEN = 'soc365.gate.v1';
...
const Auth = {
  ...
  async init() {
    ...
    /* Впервые на этом устройстве — сначала «что это» (§13.3),
       и только потом форма. Признак хранится своим ключом, а не
       выводится из наличия `soc365.v1`: прогресс появляется ПОСЛЕ
       входа, то есть по нему первый экран определялся бы уже
       поздно. Читается защитно — приватное окно и выключенное
       хранилище это нормальный режим, а не повод упасть. */
    this.go(this.seen() ? 'signin' : 'intro');
  },

  /** Видел ли этот браузер первый экран. */
  seen() {
    try { return localStorage.getItem(GATE_SEEN) === '1'; } catch (e) { return false; }
  },
  markSeen() {
    try { localStorage.setItem(GATE_SEEN, '1'); } catch (e) { /* приватное окно */ }
  },
  ...
  render() {
    ...
    /* ПЕРВЫЙ ЭКРАН ДЛЯ ЧУЖОГО ЧЕЛОВЕКА (§13.3).
       До 02.09.2026 пришедший по ссылке видел форму входа и больше
       ничего: ни что это, ни зачем. Экран отвечает на «что это»
       и не делает вида, что он лендинг — §13.3 просила именно
       десять секунд, а не страницу с отзывами.

       Текст ПРОДУКТОВЫЙ, а не трековый, и это не лень. Трек здесь
       ещё не выбран и список треков не загружен; вписать сюда
       «до junior SOC-аналитика» значило бы прибить первый экран
       к одному треку и сломать §4 — второй трек обязан лечь
       без правок кода. Что делает продукт, верно для любого трека;
       что обещает конкретный трек, человек прочтёт на его карточке
       экраном позже.

       Про бесплатность сказано прямо и один раз: §7 требует, чтобы
       ядро не резалось, а человек об этом знал до регистрации,
       а не после. */
    if (s === 'intro') {
      h = `<p class="gate-lead">План на год, разложенный по неделям.
           Задачи, часы, чеклист дня и таймер — чтобы «учить кибербез»
           превратилось в то, что видно на календаре.</p>
        <ul class="gate-list">
          <li><b>52 недели</b> — тема, задачи и часы на каждую</li>
  ...
  bind() {
    ...
    Array.from(document.querySelectorAll('[data-go]')).forEach(b => {
      ...
      b.onclick = () => {
        ...
        /* Помечаем при УХОДЕ с экрана, а не при показе: иначе человек,
           закрывший вкладку на первом экране, второй раз попал бы
           сразу на форму и «что это» не прочитал бы никогда. */
        if (this.screen === 'intro') this.markSeen();
