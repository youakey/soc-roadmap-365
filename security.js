...
/** Существующий календарный день в формате `YYYY-MM-DD`.
 *
 *  Живёт ЗДЕСЬ, а не в content.js и не в person.js, по правилу
 *  §12.1-ter: правило, зашитое в двух местах, однажды правят
 *  в одном. Дату проверяют оба — содержание трека приходит из базы,
 *  персональный старт из анкеты, — и разойтись им нельзя.
 *
 *  Одной регулярки мало, и это найдено замером, а не рассуждением
 *  (§12.7-bis). `2026-02-31` регулярке соответствует, такого дня нет,
 *  а `Date.parse` его НЕ отвергает: при неудаче строгого ISO-разбора
 *  V8 уходит в нестрогий legacy-парсер и молча отдаёт 3 марта. То есть
 *  негодная дата превращается в правдоподобную без единой ошибки —
 *  ровно тот почерк, за который платила §12.1.
 *
 *  Поэтому проверка круговая: разобрали, собрали обратно, сравнили
 *  посимвольно. День, который не пережил круг, не существует. */
function isDay(s) {
...
/* ── 5-bis. ГРАНИЦЫ ЗАПИСИ, СОГЛАСОВАННЫЕ С СЕРВЕРОМ (§8) ───
   Живут здесь по той же причине, что и isDay: правило, зашитое
   в двух местах, однажды правят в одном (§12.1-ter).

   TEXT_MAX — та же самая двухтысячная граница, что стоит в блоке
   14.2 `supabase.sql`. Сервер отвергает запись, где хоть одна
   строка внутри payload длиннее; клиент обязан не давать её
   набрать, иначе человек напишет заметку и потеряет её на
   отправке. Меняешь одно — грепни второе.

   WRITE_GAP_MS — серверный порог частоты из блока 14.1. Клиент
   держится ВЫШЕ него собственными дебаунсами (1500 и 2500 мс);
   константа нужна для повторной отправки после отказа. */
const TEXT_MAX = 2000;
const WRITE_GAP_MS = 1000;

/** Отказ «слишком часто» от блока 14.1, а не настоящая ошибка.
   Помечен `hint`, а не текстом сообщения: текст переводится
   и переписывается, пометка — нет. */
function isRateLimit(e) {
  return !!e && (own(e, 'hint', '') === 'RATE_LIMIT' || own(e, 'code', '') === 'RATE_LIMIT');
}
