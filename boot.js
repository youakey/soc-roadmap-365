...
(function () {
  ...
  function frame() {
    ...
    for (i = 0; i < P.length; i++) {
      ...
      if (phase === 'in') {
        ...
        al = Math.max(0, 1 - Math.pow(bt, 1.7) * 1.05);
    ...
    /* Хроматическое разделение на взрыве: тот же кадр, сдвинутый
       и подкрашенный. Дёшево, потому что это одна операция над уже
       нарисованным, а не второй проход по точкам. */
    if (phase === 'blast' && bt > 0.16 && !slow) {
      var off = (bt - 0.16) * 26;
      ctx.globalAlpha = 0.22 * (1 - bt);
      try {
  ...
  function kill() {
    ...
    for (var i = 0; i < P.length; i++) {
      ...
      var s2 = rnd(0.55, 1.9) / d;
      p.vx = p.x * s2 * 0.020 + rnd(-0.06, 0.06);
      p.vy = p.y * s2 * 0.020 + rnd(-0.06, 0.06);
      p.vz = p.z * s2 * 0.020;
    }
