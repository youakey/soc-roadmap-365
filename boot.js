...
(function () {
  ...
  function frame() {
    ...
    /* Условие `!slow` тут ОБЯЗАТЕЛЬНО, и его отсутствие — дефект,
       найденный замером 24.08.2026 (§12.7-bis). Оба входа в цикл
       ниже проверяют `slow`, а хвост — нет: `frame()`, вызванная
       ради «одного статичного кадра», заказывала следующий кадр
       сама, и цикл крутился бесконечно. То есть `prefers-reduced-
       motion` и узкий экран получали ровно то, от чего их берегли:
       непрерывную анимацию, только на 890 точках вместо 13 317.

       Ни одной ошибки при этом не возникало, а число точек
       в `stat()` было верным — то самое «правдоподобное число»,
       которым §12.1-ter уже платила. */
    if (!slow) raf = requestAnimationFrame(frame);
  }
